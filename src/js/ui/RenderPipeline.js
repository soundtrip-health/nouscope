...
import { WebglLineRoll } from 'webgl-plot'
import { colorVar, colorVars } from './palette'

export const OUT_N = 280   // output columns across each plot (matches canvas width)
...
// the store (see the class doc below), recreating the roll well before that
...
/**
 * RenderPipeline — the panel-drawing logic shared by `AnalysisDisplay` (the
 * original single-session view) and `MultiTrackDisplay` (one lane in the
 * Multi-Track tab). Both render the same 8 panels (EEG/PPG/IMU/bands/MSE +
 * 3 spectrograms) from a `SessionStore` at an arbitrary playhead — seekable
 * by construction, not read off a manager's live ring buffers. Only how each
 * subclass resolves its canvases (page-global ids vs a scoped root) and
 * whether panels can be individually toggled (always-on vs per-track
 * enable/disable) differs; those stay in each subclass.
 *
 * A subclass is responsible for creating `this._eegGL`/`this._eegPlot`/etc.
 * (via its own `init()`), maintaining `this._enabled` (a `Set` of active
 * panel keys — always every key for a view with no panel-select menu) and
 * `this._inited`, and implementing `_activeBands()`. It then inherits
 * `renderAt()`, `resize()`, and every panel renderer below unchanged.
 *
 * Every call to `renderAt()` clears each canvas and redraws it from scratch.
...
export default class RenderPipeline {
  ...
  /** Which normalized bands to show a legend row for; null shows every band. */
  _activeBands() {
    throw new Error('RenderPipeline subclasses must implement _activeBands()')
  }
  ...
  resize() {
    ...
    for (const gl of [this._eegGL, this._bandGL, this._mseGL, this._ppgGL, this._imuGL]) {
      if (!gl) continue   // panel disabled/not yet created — no context to size
      const c = gl.canvas
  ...
  renderAt(store, cursor) {
    ...
    // combine that with the panel's own enabled/disabled state — either reason
    // hides the section so it doesn't read as a broken/missing panel.
   ...
   * tracks in the Multi-Track tab. Cleared to fully transparent on reuse so an
   * unpainted (out-of-tolerance) column doesn't carry over stale pixels from a
   * previous frame — a freshly created `ImageData` is already zeroed, so only
   * the reuse path needs the clear.
   */
  ...
  _renderReadouts(store, t) {
    ...
    const active = this._activeBands()
    for (let i = 0; i < 5; i++) {
